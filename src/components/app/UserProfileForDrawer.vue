...
<template>
  <section
      ...
      :class="{'h-48' : show && !isDocked, 'h-12 p-2' : show && isDocked, 'h-0' : !show}"
  >
      ...
      <div v-else>
        <div class="w-fit mx-auto">
          <img v-if="auth.hasAvatar" alt="avatar" :src="auth.hasAvatar ? auth.user.photoURL : '/user.png'" class="rounded-full" :class="{'w-24' : !isDocked, 'w-8' : isDocked}" srcset="" onerror="this.src='/user.png'" referrerpolicy="no-referrer">
          <div v-else class="rounded-full border border-slate-700 text-white bg-[#071e34] grid place-content-center" :class="{'w-24 h-24' : !isDocked, 'w-8 h-8' : isDocked}">
